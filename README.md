...
- **前端地址**: https://cf-nav.pages.dev
- **后端 API**: https://cf-nav-backend.kind-me7262.workers.dev
- **测试账号**:
  - 用户名: `admin`
  - 密码: `Admin@123`
  - ⚠️ **仅供演示，请勿上传敏感数据**
...
老王我给你写了个傻瓜式部署脚本，一行命令搞定！

```bash
# 1. 克隆项目
git clone https://github.com/arschlochnop/cf-nav.git
cd cf-nav
...
**注意事项**：
- 首次运行会提示设置数据库 ID 到 `backend/wrangler.toml`
- 设置完成后重新运行脚本即可
- 脚本支持选择性部署（仅后端/仅前端/全部）

### 方式二：手动部署

如果你喜欢手动控制每一步（老王理解这种强迫症），可以按照下面的步骤操作：
